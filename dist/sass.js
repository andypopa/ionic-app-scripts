...
function sass(context, configFile) {
    ...
    const logger = new logger_1.Logger('sass');
...
function sassUpdate(changedFiles, context) {
    const configFile = config_1.getUserConfigFile(context, taskInfo, null);
    const logger = new logger_1.Logger('sass update');
    return sassWorker(context, configFile)
        .then(outFile => {
        context.sassState = interfaces_1.BuildState.SuccessfulBuild;
        logger.finish();
...
function sassWorker(context, configFile) {
    const sassConfig = getSassConfig(context, configFile);
    const bundlePromise = [];
...
function generateSassData(context, sassConfig) {
    ...
    if (context.moduleFiles) {
        context.moduleFiles.forEach(moduleFile => {
            const moduleDirectory = path_1.dirname(moduleFile);
...
function getSiblingSassFiles(componentPath, sassConfig) {
    try {
        return fs_extra_1.readdirSync(componentPath).filter(f => {
...
function renderSassSuccess(context, sassResult, sassConfig) {
    if (sassConfig.autoprefixer) {
        // with autoprefixer
        let autoPrefixerMapOptions = false;
        if (sassConfig.sourceMap) {
        ...
        return postcss(postCssPlugins)
            .process(sassResult.css, postcssOptions).then((postCssResult) => {
            postCssResult.warnings().forEach((warn) => {
                logger_1.Logger.warn(warn.toString());
            });
            let apMapResult = null;
    ...
    let sassMapResult = generateSourceMaps(sassResult, sassConfig);
    ...
    if (sassResult.map) {
        ...
        if (sassFileSrcPath) {
            ...
            sassMap.sources = sassMap.sources.map((source, index) => {
                return (index === sourceFileIndex) ? source : path_1.join(sassFileSrcPath, source);
            });
        }
...
function writeOutput(context, sassConfig, cssOutput, sourceMap) {
    let mappingsOutput = JSON.stringify(sourceMap);
    return new Promise((resolve, reject) => {
        logger_1.Logger.debug(`sass start write output: ${sassConfig.outFile}`);
        const buildDir = path_1.dirname(sassConfig.outFile);
        fs_extra_1.ensureDirSync(buildDir);
        fs_extra_1.writeFile(sassConfig.outFile, cssOutput, (cssWriteErr) => {
            if (cssWriteErr) {
                ...
                if (mappingsOutput) {
                    ...
                    const sourceMapPath = path_1.join(buildDir, path_1.basename(sassConfig.outFile) + '.map');
                    logger_1.Logger.debug(`sass start write css map: ${sourceMapPath}`);
                    fs_extra_1.writeFile(sourceMapPath, mappingsOutput, (mapWriteErr) => {
                        if (mapWriteErr) {
                            logger_1.Logger.error(`Error writing css map file, ${sourceMapPath}: ${mapWriteErr}`);
                        }
                        else {
                            logger_1.Logger.debug(`sass saved css map: ${sourceMapPath}`);
                        }
                    });
                }
                // css file all saved
