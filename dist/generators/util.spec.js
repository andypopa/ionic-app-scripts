...
const globUtils = require("../util/glob-util");
...
describe('util', () => {
    describe('hydrateRequest', () => {
        it('should take a component request and return a hydrated component request', () => {
            // arrange
            const baseDir = path_1.join(process.cwd(), 'someDir', 'project');
            const componentsDir = path_1.join(baseDir, 'src', 'components');
            const context = {
            ...
            const templateDir = path_1.join(baseDir, 'node_modules', 'ionic-angular', 'templates');
        ...
        it('should take a page request and return a hydrated page request', () => {
            // arrange
            const baseDir = path_1.join(process.cwd(), 'someDir', 'project');
            const pagesDir = path_1.join(baseDir, 'src', 'pages');
            const context = {
                pagesDir: pagesDir
            };
            const request = {
...
$TAB_CONTENT
    ...
    describe('writeGeneratedFiles', () => {
        it('should return the list of files generated', () => {
            ...
            const fileName = 'settings-view';
            const dirToWrite = path_1.join('/Users/noone/project/src/components', fileName);
            map.set(path_1.join(templateDir, knownValues[0]), fileContent);
            map.set(path_1.join(templateDir, knownValues[1]), fileContent);
            map.set(path_1.join(templateDir, knownValues[2]), fileContent);
            map.set(path_1.join(templateDir, knownValues[3]), fileContent);
            ...
            return promise.then((filesCreated) => {
                const fileExtensions = knownValues.map(knownValue => path_1.basename(knownValue, GeneratorConstants.KNOWN_FILE_EXTENSION));
    ...
    describe('getNgModules', () => {
        let context;
        const componentsDir = path_1.join(process.cwd(), 'path', 'to', 'components');
        const directivesDir = path_1.join(process.cwd(), 'path', 'to', 'directives');
        const pagesDir = path_1.join(process.cwd(), 'path', 'to', 'pages');
        const pipesDir = path_1.join(process.cwd(), 'path', 'to', 'pipes');
        const providersDir = path_1.join(process.cwd(), 'path', 'to', 'providers');
        beforeEach(() => {
            context = {
                componentsDir,
                directivesDir,
