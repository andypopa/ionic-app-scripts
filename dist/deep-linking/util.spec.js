...
const tsUtils = require("../util/typescript-utils");
describe('util', () => {
    describe('filterTypescriptFilesForDeepLinks', () => {
        it('should return a list of files that are in the directory specified for deeplinking', () => {
            const pagesDir = path_1.join(process.cwd(), 'myApp', 'src', 'pages');
            const knownFileContent = 'Some string';
            const pageOneTs = path_1.join(pagesDir, 'page-one', 'page-one.ts');
            const pageOneHtml = path_1.join(pagesDir, 'page-one', 'page-one.html');
            const pageOneModule = path_1.join(pagesDir, 'page-one', 'page-one.module.ts');
            const pageTwoTs = path_1.join(pagesDir, 'page-two', 'page-two.ts');
            const pageTwoHtml = path_1.join(pagesDir, 'page-two', 'page-two.html');
            const pageTwoModule = path_1.join(pagesDir, 'page-two', 'page-two.module.ts');
            const pageThreeTs = path_1.join(pagesDir, 'page-three', 'page-three.ts');
            const pageThreeHtml = path_1.join(pagesDir, 'page-three', 'page-three.html');
            const pageThreeModule = path_1.join(pagesDir, 'page-three', 'page-three.module.ts');
            const someOtherFile = path_1.join('Users', 'hans-gruber', 'test.ts');
            const fileCache = new file_cache_1.FileCache();
            ...
            spyOn(helpers, helpers.getStringPropertyValue.name).and.callFake((input) => {
...
export class HomePage {
  ...
  nextPage() {
    ...
    console.log()
      ...
      `;
            const knownPath = path_1.join(process.cwd(), 'some', 'fake', 'path');
...
export class HomePage {
  ...
  nextPage() {
    ...
    console.log()
      ...
      `;
            const knownPath = path_1.join(process.cwd(), 'myApp', 'src', 'pages', 'about.ts');
            const sourceFile = tsUtils.getTypescriptSourceFile(knownPath, knownContent);
            const result = util.getDeepLinkDecoratorContentForSourceFile(sourceFile);
            expect(result.name).toEqual('HomePage');
...
export class HomePage {
  ...
  nextPage() {
    ...
    console.log()
      ...
      `;
            const knownPath = path_1.join(process.cwd(), 'some', 'fake', 'path.ts');
            const sourceFile = tsUtils.getTypescriptSourceFile(knownPath, knownContent);
            const result = util.getDeepLinkDecoratorContentForSourceFile(sourceFile);
            expect(result.name).toEqual('HomePage');
            expect(result.segment).toEqual('path');
  ...
  return source;
    ...
    describe('getNgModuleDataFromCorrespondingPage', () => {
        it('should call the file cache with the path to an ngmodule', () => {
            const basePath = path_1.join(process.cwd(), 'some', 'fake', 'path');
            const pagePath = path_1.join(basePath, 'my-page', 'my-page.ts');
            const ngModulePath = path_1.join(basePath, 'my-page', 'my-page.module.ts');
    ...
    describe('getRelativePathToPageNgModuleFromAppNgModule', () => {
        it('should return the relative path', () => {
            const prefix = path_1.join(process.cwd(), 'myApp', 'src');
            const appNgModulePath = path_1.join(prefix, 'app', 'app.module.ts');
            const pageNgModulePath = path_1.join(prefix, 'pages', 'page-one', 'page-one.module.ts');
    ...
    describe('getNgModuleDataFromPage', () => {
        it('should throw when NgModule is not in cache and create default ngModule flag is off', () => {
            const prefix = path_1.join(process.cwd(), 'myApp', 'src');
            const appNgModulePath = path_1.join(prefix, 'app', 'app.module.ts');
            const pagePath = path_1.join(prefix, 'pages', 'page-one', 'page-one.ts');
...
export class PageThreeModule {
      ...
      `;
            const prefix = path_1.join(process.cwd(), 'myApp', 'src');
            const appNgModulePath = path_1.join(prefix, 'app', 'app.module.ts');
            const pageOneNgModulePath = path_1.join(prefix, 'pages', 'page-one', 'page-one.module.ts');
            const pageOnePath = path_1.join(prefix, 'pages', 'page-one', 'page-one.ts');
            const pageTwoNgModulePath = path_1.join(prefix, 'pages', 'page-two', 'page-two.module.ts');
            const pageTwoPath = path_1.join(prefix, 'pages', 'page-two', 'page-two.ts');
            const pageSettingsNgModulePath = path_1.join(prefix, 'pages', 'settings-page', 'settings-page.module.ts');
            const pageSettingsPath = path_1.join(prefix, 'pages', 'settings-page', 'settings-page.ts');
            const fileCache = new file_cache_1.FileCache();
...
export class PageThreeModule {
      ...
      `;
            const srcDir = path_1.join(process.cwd(), 'myApp', 'src');
            const appNgModulePath = path_1.join(srcDir, 'app', 'app.module.ts');
            const pageOneNgModulePath = path_1.join(srcDir, 'pages', 'page-one', 'page-one.module.ts');
            const pageOnePath = path_1.join(srcDir, 'pages', 'page-one', 'page-one.ts');
            const pageTwoNgModulePath = path_1.join(srcDir, 'pages', 'page-two', 'page-two.module.ts');
            const pageTwoPath = path_1.join(srcDir, 'pages', 'page-two', 'page-two.ts');
            const pageSettingsNgModulePath = path_1.join(srcDir, 'pages', 'settings-page', 'settings-page.module.ts');
            const pageSettingsPath = path_1.join(srcDir, 'pages', 'settings-page', 'settings-page.ts');
            const fileCache = new file_cache_1.FileCache();
            fileCache.set(pageOnePath, { path: pageOnePath, content: pageOneContent });
            fileCache.set(pageOneNgModulePath, { path: pageOneNgModulePath, content: pageOneNgModuleContent });
            fileCache.set(pageTwoPath, { path: pageTwoPath, content: pageTwoContent });
...
export class PageThreeModule {
      ...
      `;
            ...
            const pageTwoNgModulePath = path_1.join(srcDir, 'pages', 'page-two', 'page-two.module.ts');
            const pageTwoPath = path_1.join(srcDir, 'pages', 'page-two', 'page-two.ts');
            const pageSettingsNgModulePath = path_1.join(srcDir, 'pages', 'settings-page', 'fake-dir', 'settings-page.module.ts');
            const pageSettingsPath = path_1.join(srcDir, 'pages', 'settings-page', 'fake-dir', 'settings-page.ts');
            const fileCache = new file_cache_1.FileCache();
            fileCache.set(pageOnePath, { path: pageOnePath, content: pageOneContent });
            fileCache.set(pageOneNgModulePath, { path: pageOneNgModulePath, content: pageOneNgModuleContent });
            fileCache.set(pageTwoPath, { path: pageTwoPath, content: pageTwoContent });
...
export class PageThreeModule {
      ...
      `;
            const srcDir = path_1.join(process.cwd(), 'myApp', 'src');
            const appNgModulePath = path_1.join(srcDir, 'app', 'app.module.ts');
            const pageOneNgModulePath = path_1.join(srcDir, 'pages', 'page-one', 'page-one.not-module.ts');
            const pageOnePath = path_1.join(srcDir, 'pages', 'page-one', 'page-one.ts');
            const pageTwoNgModulePath = path_1.join(srcDir, 'pages', 'page-two', 'page-two.module.ts');
            const pageTwoPath = path_1.join(srcDir, 'pages', 'page-two', 'page-two.ts');
            const pageSettingsNgModulePath = path_1.join(srcDir, 'pages', 'settings-page', 'fake-dir', 'settings-page.module.ts');
...
export class AppModule {}
      `;
            const knownPath = path_1.join(process.cwd(), 'idk', 'some', 'fake', 'path');
            const result = util.hasExistingDeepLinkConfig(knownPath, knownContent);
...
export class AppModule {}
      `;
            const knownPath = path_1.join('some', 'fake', 'path');
...
`;
            const knownFilePath = path_1.join(process.cwd(), 'myApp', 'src', 'pages', 'page-one', 'page-one.ts');
            const knownClassName = 'PageOne';
...
`;
            const knownAppNgModulePath = path_1.join(process.cwd(), 'myApp', 'src', 'app.module.ts');
