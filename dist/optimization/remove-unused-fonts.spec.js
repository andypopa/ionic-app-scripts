...
const remove_unused_fonts_1 = require("./remove-unused-fonts");
const helpers = require("../util/helpers");
describe('Remove Fonts', () => {
    describe('removeUnusedFonts', () => {
        it('should not purge any fonts when target is not cordova', () => {
            const fakeFontDirPath = path_1.join(process.cwd(), 'www', 'assets', 'fonts');
            ...
            return remove_unused_fonts_1.removeUnusedFonts({ target: 'notCordova', platform: 'web' }).then(() => {
        ...
        it('should purge all non-woffs for ionicons and roboto, and then all of noto-sans for ios', () => {
            ...
            return remove_unused_fonts_1.removeUnusedFonts({ target: 'cordova', platform: 'ios' }).then(() => {
                expect(helpers.readDirAsync).toHaveBeenCalledWith(fakeFontDirPath);
        ...
        it('should purge all non-woffs for ionicons, all of roboto and noto-sans for android', () => {
            ...
            spyOn(helpers, helpers.readDirAsync.name).and.returnValue(Promise.resolve(getMockFontDirData()));
            const unlinkSpy = spyOn(helpers, helpers.unlinkAsync.name).and.returnValue(Promise.resolve());
            return remove_unused_fonts_1.removeUnusedFonts({ target: 'cordova', platform: 'android' }).then(() => {
                expect(helpers.readDirAsync).toHaveBeenCalledWith(fakeFontDirPath);
                expect(unlinkSpy.calls.all()[0].args[0]).toEqual(path_1.join(fakeFontDirPath, 'ionicons.eot'));
                expect(unlinkSpy.calls.all()[1].args[0]).toEqual(path_1.join(fakeFontDirPath, 'ionicons.scss'));
                expect(unlinkSpy.calls.all()[2].args[0]).toEqual(path_1.join(fakeFontDirPath, 'ionicons.svg'));
        ...
        it('should purge all non-woffs for ionicons, all of roboto and noto-sans for windows', () => {
            ...
            spyOn(helpers, helpers.readDirAsync.name).and.returnValue(Promise.resolve(getMockFontDirData()));
            const unlinkSpy = spyOn(helpers, helpers.unlinkAsync.name).and.returnValue(Promise.resolve());
            return remove_unused_fonts_1.removeUnusedFonts({ target: 'cordova', platform: 'windows' }).then(() => {
                expect(helpers.readDirAsync).toHaveBeenCalledWith(fakeFontDirPath);
                expect(helpers.unlinkAsync).not.toHaveBeenCalled();
            });
        });
